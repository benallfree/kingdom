...
var require_url_parse = __commonJS({
  'node_modules/url-parse/index.js'(exports2, module2) {
    ...
    function Url(address, location, parser2) {
      ...
      for (; i < instructions.length; i++) {
        ...
        if (parse3 !== parse3) {
          ...
          index =
            parse3 === '@'
              ? address.lastIndexOf(parse3)
              : address.indexOf(parse3)
          if (~index) {
    ...
    function toString2(stringify7) {
      if (!stringify7 || 'function' !== typeof stringify7)
        stringify7 = qs.stringify
      var query,
        url = this,
        host = url.host,
...
`
    ...
    try {
      if (typeof Object.defineProperty === 'function') {
        Object.defineProperty(exports2.escapeXML, 'toString', {
...
var require_ejs = __commonJS({
  '../pocketbase-ejs/lib/ejs.js'(exports2) {
    ...
    function Template(text, optsParam) {
      ...
      options2.openDelimiter =
        opts.openDelimiter || exports2.openDelimiter || _DEFAULT_OPEN_DELIMITER
      options2.closeDelimiter =
        opts.closeDelimiter ||
        exports2.closeDelimiter ||
        _DEFAULT_CLOSE_DELIMITER
      options2.delimiter =
        opts.delimiter || exports2.delimiter || _DEFAULT_DELIMITER
  ...
  'node_modules/js-yaml/lib/js-yaml/loader.js'(exports2, module2) {
    ...
    function safeLoadAll(input, iterator, options2) {
      ...
      return loadAll(
        input,
        iterator,
        common.extend({ schema: DEFAULT_SAFE_SCHEMA }, options2)
      )
    }
...
var require_dumper = __commonJS({
  'node_modules/js-yaml/lib/js-yaml/dumper.js'(exports2, module2) {
    ...
    function State(options2) {
      ...
      this.flowLevel = common.isNothing(options2['flowLevel'])
        ? -1
        : options2['flowLevel']
...
`
  ...
  tablecell(token2) {
    ...
    const tag2 = token2.align
      ? `<${type} align="${token2.align}">`
      : `<${type}>`
    return (
      tag2 +
      content +
...
`
  ...
  text(token2) {
    return 'tokens' in token2 && token2.tokens
      ? this.parser.parseInline(token2.tokens)
      : token2.text
  }
}
...
import_pocketbase_ejs.default.resolveInclude = function (
  includePath,
  templatePath,
  isDir
) {
  ;(0, import_pocketbase_log3.dbg)(`resolveInclude`, {
    name: includePath,
    filename: templatePath,
    isDir,
  })
  if (includePath.startsWith('/')) {
    return import_pocketbase_node2.path.resolve(
      pagesRoot,
      `_private`,
      includePath
    )
  ...
  while (currentPath.length >= pagesRoot.length) {
    const attemptPath = import_pocketbase_node2.path.resolve(
      currentPath,
      `_private`,
      includePath
    )
...
var renderFile = (fname, api) => {
    ...
    { ...api, api },
    {
      ...
      includer: (path3, filename) => {
        ...
        if ($filepath.ext(filename) === '.md') {
          const markdown = import_pocketbase_node2.fs.readFileSync(
            filename,
            'utf8'
          )
  ...
  if (typeof content !== 'string') {
    return (0, import_pocketbase_stringify3.stringify)(content)
  }
  return content
}
  ...
  switch (options2.arrayFormat) {
    case 'index': {
      return (key) => (result, value) => {
        ...
        if (
          value === void 0 ||
          (options2.skipNull && value === null) ||
          (options2.skipEmptyString && value === '')
        ) {
          return result
        }
        if (value === null) {
        ...
        return [
          ...
          [
            encode(key, options2),
            '[',
            encode(index, options2),
            ']=',
            encode(value, options2),
          ].join(''),
        ]
      }
    }
    ...
    case 'bracket-separator': {
      const keyValueSeparator =
        options2.arrayFormat === 'bracket-separator' ? '[]=' : '='
      return (key) => (result, value) => {
        if (
          value === void 0 ||
          (options2.skipNull && value === null) ||
        ...
        return [
          [result, encode(value, options2)].join(options2.arrayFormatSeparator),
        ]
      }
    }
    default: {
  ...
  switch (options2.arrayFormat) {
    ...
    case 'separator': {
      return (key, value, accumulator) => {
        const isArray =
          typeof value === 'string' &&
          value.includes(options2.arrayFormatSeparator)
        const isEncodedArray =
          typeof value === 'string' &&
          !isArray &&
          decode2(value, options2).includes(options2.arrayFormatSeparator)
        value = isEncodedArray ? decode2(value, options2) : value
        const newValue =
          isArray || isEncodedArray
            ? value
                .split(options2.arrayFormatSeparator)
                .map((item) => decode2(item, options2))
            : value === null
              ? value
              : decode2(value, options2)
    ...
    case 'bracket-separator': {
      return (key, value, accumulator) => {
        ...
        const arrayValue =
          value === null
            ? []
            : decode2(value, options2).split(options2.arrayFormatSeparator)
        if (accumulator[key] === void 0) {
...
function parse(query, options2) {
  ...
  for (const parameter of query.split('&')) {
    ...
    const parameter_ = options2.decode
  ...
  for (const [key, value] of Object.entries(returnValue)) {
    if (
      typeof value === 'object' &&
      value !== null &&
      options2.types[key] !== 'string'
    ) {
      for (const [key2, value2] of Object.entries(value)) {
        const type = options2.types[key]
          ? options2.types[key].replace('[]', '')
          : void 0
  ...
  return (
    options2.sort === true
      ...
      : Object.keys(returnValue).sort(options2.sort)
  ).reduce((result, key) => {
...
function stringify4(object, options2) {
  if (!object) {
    return ''
  }
  options2 = {
  ...
  const shouldFilter = (key) =>
    (options2.skipNull && isNullOrUndefined(object[key])) ||
    (options2.skipEmptyString && object[key] === '')
...
function parseUrl(url, options2) {
  ...
  return {
    ...
    ...(options2 && options2.parseFragmentIdentifier && hash
      ? { fragmentIdentifier: decode2(hash, options2) }
      : {}),
  }
}
function stringifyUrl(object, options2) {
  ...
  let queryString = stringify4(query, options2)
  if (queryString) {
  ...
  if (typeof object.fragmentIdentifier === 'string') {
    ...
    hash = options2[encodeFragmentIdentifier]
...
var MiddlewareHandler = (request, response, next) => {
  ...
  try {
    ...
    const api = {
      ...
      slots: {},
      asset: (path3) => {
        const shortAssetPath = path3.startsWith('/')
          ? path3
          : $filepath.join(route.assetPrefix, path3)
        const fullAssetPath = path3.startsWith('/')
          ? path3
          : $filepath.join(
        ...
        const assetRoute = parseRoute(
          new import_url_parse.default(fullAssetPath),
          routes
        )
